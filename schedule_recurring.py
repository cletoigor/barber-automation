...
def create_calendar_event(date_str, hour, dry_run=False):
    """Cria evento no Google Calendar após agendamento confirmado no app."""
    h, m = map(int, hour.split(":"))
    end_total = h * 60 + m + 30
    end_hour = f"{end_total // 60:02d}:{end_total % 60:02d}"

    prompt = (
        f"Create a Google Calendar event on {date_str} "
        f"with summary 'Barba — the barbershop (barber A)' "
        f"starting at {hour} and ending at {end_hour} (timezone America/Sao_Paulo). "
        f"Set color to Tangerine (colorId 6). "
        f"Add popup reminders: 1440 minutes (24h) and 60 minutes (1h) before."
    )
    ok = _run_claude(
        prompt,
        "mcp__claude_ai_Google_Calendar__create_event",
        dry_run_label=f"Criaria evento no Calendar: {date_str} {hour}–{end_hour}",
        dry_run=dry_run,
    )
    if ok:
        print(f"    ✓ Google Calendar: evento criado {date_str} {hour}–{end_hour}")
    else:
        print(f"    ⚠ Falha ao criar evento no Calendar.")
...
def notify_no_slot(date_str, dry_run=False):
    ...
    prompt = (
        ...
        f"and description 'Nenhum horário disponível entre 09:30 e 11:00 com barber A na the barbershop. Agendar pelo app.' "
...
def main():
    ...
    for d in dates:
        ...
        create_calendar_event(date_str, hour, dry_run=args.dry_run)
